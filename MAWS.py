...
def times(number, array):
    ...
    output = [
        math.fmul(
            math.mpmathify(elem),
            math.mpmathify(number)) for elem in array]
    return output
...
def get_offset(positions_old, positions):
    vec_a = (positions[len(positions_old) - 1] -
             positions[len(positions_old) - 2])
    vec_b = (positions_old[len(positions_old) - 1] - positions_old[-2])
    alpha = math.acos(sum([alem.value_in_unit(unit.angstroms) *
                           blem.value_in_unit(unit.angstroms) for alem, blem in zip(vec_a, vec_b)]) /
                      (np.linalg.norm(vec_a.value_in_unit(unit.angstroms)) *
                       np.linalg.norm(vec_b.value_in_unit(unit.angstroms))))
    alpha_t = 0.
    #alpha_t = math.pi-113.3*math.pi/360.
    d_alpha = alpha_t - alpha
    axis = cross(
        vec_a.value_in_unit(
            unit.angstroms), vec_b.value_in_unit(
            unit.angstroms)) / np.linalg.norm(
                cross(
                    vec_a.value_in_unit(
                        unit.angstroms), vec_b.value_in_unit(
                            unit.angstroms)))
...
def get_offset_five(positions_old, positions, ligand_length):
    vec_a = (positions[ligand_length +
                       (len(positions) -
                        len(positions_old)) +
                       1] -
             positions[ligand_length +
                       (len(positions) -
                        len(positions_old))])
...
def position_aptamer_five(positions_old, positions, ligand_length):
    alpha, axis, offset, vec_a, vec_b = get_offset_five(
    ...
    for j in range(ligand_length, ligand_length +
                   (len(positions) - len(positions_old))):
        roted = np.dot(
            np.array(
                positions[j].value_in_unit(
                    unit.angstrom)),
...
def stratify(pos, energies, size, phi_size):
    ...
    varx1 = np.math.sqrt(np.array([np.power(energies[l] - meanx1, 2) for l in range(
        len(energies)) if pos[l][0] < 0.]).sum() / (countx1 - 1 + 1e-20))
    print(varx1)
    vary1 = np.math.sqrt(np.array([np.power(energies[l] - meany1, 2) for l in range(
        len(energies)) if pos[l][1] < 0.]).sum() / (county1 - 1 + 1e-20))
    varz1 = np.math.sqrt(np.array([np.power(energies[l] - meanz1, 2) for l in range(
        len(energies)) if pos[l][2] < 0.]).sum() / (countz1 - 1 + 1e-20))
    vari1 = np.math.sqrt(np.array([np.power(energies[l] - meani1, 2) for l in range(
        len(energies)) if pos[l][3] < 0.]).sum() / (counti1 - 1 + 1e-20))
    varj1 = np.math.sqrt(np.array([np.power(energies[l] - meanj1, 2) for l in range(
        len(energies)) if pos[l][4] < 0.]).sum() / (countj1 - 1 + 1e-20))
    vark1 = np.math.sqrt(np.array([np.power(energies[l] - meank1, 2) for l in range(
        len(energies)) if pos[l][5] < 0.]).sum() / (countk1 - 1 + 1e-20))
    varphi1 = np.math.sqrt(np.array([np.power(energies[l] - meanphi1, 2) for l in range(
        len(energies)) if pos[l][6] < 0.]).sum() / (countphi1 - 1 + 1e-20))
    varx2 = np.math.sqrt(np.array([np.power(energies[l] - meanx2, 2) for l in range(
        len(energies)) if pos[l][0] >= 0.]).sum() / (countx2 - 1 + 1e-20))
    vary2 = np.math.sqrt(np.array([np.power(energies[l] - meany2, 2) for l in range(
        len(energies)) if pos[l][1] >= 0.]).sum() / (county2 - 1 + 1e-20))
    varz2 = np.math.sqrt(np.array([np.power(energies[l] - meanz2, 2) for l in range(
        len(energies)) if pos[l][2] >= 0.]).sum() / (countz2 - 1 + 1e-20))
    vari2 = np.math.sqrt(np.array([np.power(energies[l] - meani2, 2) for l in range(
        len(energies)) if pos[l][3] >= 0.]).sum() / (counti2 - 1 + 1e-20))
    varj2 = np.math.sqrt(np.array([np.power(energies[l] - meanj2, 2) for l in range(
        len(energies)) if pos[l][4] >= 0.]).sum() / (countj2 - 1 + 1e-20))
    vark2 = np.math.sqrt(np.array([np.power(energies[l] - meank2, 2) for l in range(
        len(energies)) if pos[l][5] >= 0.]).sum() / (countk2 - 1 + 1e-20))
    varphi2 = np.math.sqrt(np.array([np.power(energies[l] - meanphi2, 2) for l in range(
        len(energies)) if pos[l][6] >= 0.]).sum() / (countphi2 - 1 + 1e-20))
...
def uniform_strat(
    ...
    distx = np.random.choice(np.array([np.random.uniform(size[0] + (l - 1) * stepx, size[0] + l * stepx)
                             for l in range(1, len(x) + 1)]), p=np.append(x[:-1], [1 - sum(x[:-1])]))
    disty = np.random.choice(np.array([np.random.uniform(size[0] + (l - 1) * stepy, size[0] + l * stepy)
                             for l in range(1, len(y) + 1)]), p=np.append(y[:-1], [1 - sum(y[:-1])]))
    distz = np.random.choice(np.array([np.random.uniform(size[0] + (l - 1) * stepz, size[0] + l * stepz)
                             for l in range(1, len(z) + 1)]), p=np.append(z[:-1], [1 - sum(z[:-1])]))
    disti = np.random.choice(np.array([np.random.uniform(phi_size[0] +
                                                         (l -
                                                          1) *
                                                         stepi, phi_size[0] +
                                                         l *
                                                         stepi) for l in range(1, len(i) +
                                                                               1)]), p=np.append(i[:-
                                                                                                   1], [1 -
                                                                                                        sum(i[:-
                                                                                                              1])]))
    distj = np.random.choice(np.array([np.random.uniform(phi_size[0] +
                                                         (l -
                                                          1) *
                                                         stepj, phi_size[0] +
                                                         l *
                                                         stepj) for l in range(1, len(j) +
                                                                               1)]), p=np.append(j[:-
                                                                                                   1], [1 -
                                                                                                        sum(j[:-
                                                                                                              1])]))
    distk = np.random.choice(np.array([np.random.uniform(phi_size[0] +
                                                         (l -
                                                          1) *
                                                         stepk, phi_size[0] +
                                                         l *
                                                         stepk) for l in range(1, len(k) +
                                                                               1)]), p=np.append(k[:-
                                                                                                   1], [1 -
                                                                                                        sum(k[:-
                                                                                                              1])]))
    distphi = np.random.choice(np.array([np.random.uniform(phi_size[0] +
                                                           (l -
                                                            1) *
                                                           stepphi, phi_size[0] +
                                                           l *
                                                           stepphi) for l in range(1, len(phi) +
                                                                                   1)]), p=np.append(phi[:-
                                                                                                         1], [1 -
                                                                                                              sum(phi[:-
                                                                                                                      1])]))
...
def energy_strata(xyz, energies):
    energies = [math.mpmathify(elem) for elem in energies]
    strata = [[energies[l] for l in range(len(energies)) if (c * xyz[l][0] <= 0) and (b * xyz[l][1] <= 0) and (a * xyz[l][2] <= 0) and (d * xyz[l][3] <= 0) and (e * xyz[l][4] <= 0) and (
        f * xyz[l][5] <= 0) and (g * xyz[l][6] <= 0)] for a in [-1, 1] for b in [-1, 1] for c in [-1, 1] for d in [-1, 1] for e in [-1, 1] for f in [-1, 1] for g in [-1, 1]]
    return strata
...
class Aptamer:
    ...
    def build_geometry(self, identifier):
        for elem in self.geometry:
            ...
            bond_dict = {
                ...
                3: torsionC5O5,
                4: torsionPO3}
            bond_dict[bond](*[identifier, residue, angle])

    def translate(self, identifier, vector, step=1):
        self.command("translate " + identifier + "." +
                     str(step + 1) + " {%s %s %s}" % tuple(vector))
    ...
    def rotate(self, identifier, angles, step=1):
        ...
        rotateZ = [math.cos(angles[0] *
                            ang_num), -
                   math.sin(angles[0] *
                            ang_num), 0, math.sin(angles[0] *
                                                  ang_num), math.cos(angles[0] *
                                                                     ang_num), 0, 0, 0, 1]
        rotateY = [math.cos(angles[1] *
                            ang_num), 0, -
                   math.sin(angles[1] *
                            ang_num), 0, 1, 0, math.sin(angles[1] *
                                                        ang_num), 0, math.cos(angles[1] *
                                                                              ang_num)]
        rotateX = [1, 0, 0, 0, math.cos(angles[2] *
                                        ang_num), -
                   math.sin(angles[2] *
                            ang_num), 0, math.sin(angles[2] *
                                                  ang_num), math.cos(angles[2] *
                                                                     ang_num)]
        antirotateZ = [math.cos(-angles[0] * ang_num),
                       -math.sin(-angles[0] * ang_num),
                       0,
                       math.sin(-angles[0] * ang_num),
                       math.cos(-angles[0] * ang_num),
                       0,
                       0,
                       0,
                       1]
        antirotateY = [math.cos(-angles[1] * ang_num),
                       0,
                       -math.sin(-angles[1] * ang_num),
                       0,
                       1,
                       0,
                       math.sin(-angles[1] * ang_num),
                       0,
                       math.cos(-angles[1] * ang_num)]
        antirotateX = [1,
                       0,
                       0,
                       0,
                       math.cos(-angles[2] * ang_num),
                       -math.sin(-angles[2] * ang_num),
                       0,
                       math.sin(-angles[2] * ang_num),
                       math.cos(-angles[2] * ang_num)]
    ...
    def rotate_axis(self, identifier, angle, axis, step):
        ...
        #self.command("transform "+identifier+""+"{ {%s %s %s} {%s %s %s} {%s %s %s} }"%tuple(rotateZ))
        self.command("transform " + identifier + "." + str(step + 1) +
                     "{ {%s %s %s} {%s %s %s} {%s %s %s} }" % tuple(rotateM))
        ...
        power(
            sum_a(
                power2(
                    ...
                    2)),
            0.5)) + padding
    box_x = max([abs(positions[i][0])
                for i in range(len(positions))]) + padding
    box_y = max([abs(positions[i][1])
                for i in range(len(positions))]) + padding
    box_z = max([abs(positions[i][2])
                for i in range(len(positions))]) + padding
...
def ligand_energy():
    ...
    lig_crd = app.AmberInpcrdFile("ligand.inpcrd")
    lig_system = lig_top.createSystem(
        nonbondedMethod=NoCutoff,
        nonbondedCutoff=10 * nanometer,
        constraints=HAngles,
        implicitSolvent=OBC1)
    lig_integrator = LangevinIntegrator(
...
def get_PO3(positions_old, positions):
    ...
    shift_forward = mm.Vec3(0, 0, 0) * unit.angstroms - \
        positions[len(positions_old) - 1]
...
def get_PO5(positions_old, positions):
    pos = positions
    vec_a = (positions[len(positions_old) + 2] -
             positions[len(positions_old) - 1])
    x, y, z = vec_a.value_in_unit(unit.angstroms)
    x, y, z = np.array([x, y, z]) / np.linalg.norm(np.array([x, y, z]))
    shift_forward = mm.Vec3(0, 0, 0) * unit.angstroms - \
...
def get_C5O5(positions_old, positions):
    pos = positions
    vec_a = (positions[len(positions_old) + 3] -
             positions[len(positions_old) + 2])
    x, y, z = vec_a.value_in_unit(unit.angstroms)
    x, y, z = np.array([x, y, z]) / np.linalg.norm(np.array([x, y, z]))
    shift_forward = mm.Vec3(0, 0, 0) * unit.angstroms - \
        positions[len(positions_old) + 3]
    phi_2 = np.random.uniform(-np.math.pi / 2, np.math.pi / 2)
    s = np.math.sin(phi_2)
    c = np.math.cos(phi_2)
    rot = np.array([[2 *
...
def get_C5(positions_old, positions):
    pos = positions
    vec_a = (positions[len(positions_old) + 6] -
             positions[len(positions_old) + 3])
    x, y, z = vec_a.value_in_unit(unit.angstroms)
    x, y, z = np.array([x, y, z]) / np.linalg.norm(np.array([x, y, z]))
    shift_forward = mm.Vec3(0, 0, 0) * unit.angstroms - \
        positions[len(positions_old) + 6]
    phi_2 = np.random.uniform(-np.math.pi / 2, np.math.pi / 2)
    s = np.math.sin(phi_2)
    c = np.math.cos(phi_2)
    rot = np.array([[2 *
...
def get_base(positions_old, positions):
    pos = positions
    vec_a = (positions[len(positions_old) + 11] -
             positions[len(positions_old) + 9])
    x, y, z = vec_a.value_in_unit(unit.angstroms)
    x, y, z = np.array([x, y, z]) / np.linalg.norm(np.array([x, y, z]))
    shift_forward = mm.Vec3(0, 0, 0) * unit.angstroms - \
        positions[len(positions_old) + 11]
    phi_2 = np.random.uniform(-np.math.pi / 2, np.math.pi / 2)
    s = np.math.sin(phi_2)
    c = np.math.cos(phi_2)
    rot = np.array([[2 *
...
def initial_sample(topology, coordinates, Nsteps, index, box=50, rang=[0, 60]):
    ...
    # print("a")
    system = aptamer_top.createSystem(
    ...
    for i in range(Nsteps):
        ...
        shift = mm.Vec3(np.random.uniform(-box, box), np.random.uniform(-box,
                        box), np.random.uniform(-box, box)) * unit.angstrom
        x = np.random.uniform(-1, 1)
        y = np.random.uniform(-1, 1)
        z = np.random.uniform(-1, 1)
        x, y, z = np.array([x, y, z]) * 1 / \
            (np.linalg.norm(np.array([x, y, z])))

        phi_2 = np.random.uniform(-np.math.pi, np.math.pi)
        x, y, z = np.array([x, y, z]) * 1 / \
            (np.linalg.norm(np.array([x, y, z])))
        xyz.append([shift[0].value_in_unit(unit.angstroms), shift[1].value_in_unit(
            unit.angstroms), shift[2].value_in_unit(unit.angstroms), x, y, z, phi_2])
        ...
        if free_E < free_E_old:
            ...
            app.PDBFile.writeModel(
                aptamer_top.topology, pos, file=fil, modelIndex=i)
            fil.close()
            del fil
...
def stratified_sample(
    ...
    for i in range(Nsteps):
        ...
        shift = [0, 0, 0]
        shift[0], shift[1], shift[2], x, y, z, phi_2 = uniform_strat(
            variances[0], variances[1], variances[2], variances[3], variances[4], variances[5], variances[6], [-box, box], [-np.math.pi, np.math.pi])
        x, y, z = np.array([x, y, z]) * 1 / \
            (np.linalg.norm(np.array([x, y, z])))
...
def mcmc_sample_five(
    ...
    heavies = [len(get_ligand(aptamer_top.topology)) +
               (len(positions) -
                len(positions_old)) -
               1, len(aptamer_crd.coordinates)]
    system = aptamer_top.createSystem(
        nonbondedMethod=app.CutoffNonPeriodic,
        nonbondedCutoff=1.2 * unit.nanometers,
        constraints=app.HBonds,
...
def initial(Ntide):
    ...
    en_pos_xyz = [
        initial_sample(
            ...
            i,
            box=sample_box,
            rang=ligand_range) for i in range(100)]
...
#    if is_3prime == 1:
    en_pos = [
        mcmc_sample(
            ...
            old_positions,
            index,
            Nsteps=_NSTEP) for index in range(10)]
#    else:
...
#    en_pos = value(en_pos_task)
    ...
    P = [math.exp(-beta * elem) / Z for elem in en]
    S = volume * math.fsum([-elem * math.log(elem * volume)
                           for elem in P]) / len(P)
                                  ...
                                  [[alem,
                                    blem.replace("3",
                                                 "").replace("N",
                                                             "5").strip() + ntide,
                                      1] for alem,
                                      blem in zip(positions,
                                                  Ntides) for ntide in [" DG3",
                                                                        " DA3",
                                                                        " DT3",
                                                                        " DC3"]])
...
def result(pos_Nt):
    ...
    for elem in pos_Nt:
        ...
        internal.unify(identifier)
        internal.command(
            "saveamberparm union Aptamer%s.prmtop Aptamer%s.inpcrd" %
            (count, count))
        time.sleep(2)
...
parser = argparse.ArgumentParser(
    description='MAWS - Make Aptamers Without SELEX',
...
parser.add_argument(
    'infile',
    metavar='INFILE',
    help='3D structure of the ligand.')
parser.add_argument(
    '-b',
    '--beta',
    type=float,
    default=0.01,
    help='lagrange multiplier beta.')
parser.add_argument('-i', '--ninit', type=int, default=200,
                    help='number of initial steps as multiple of 100.')
parser.add_argument('-s', '--nstep', type=int, default=200,
                    help='number of samples in every step after the first.')
parser.add_argument('-l', '--nmer', type=int, default=15,
                    help='The final length of the aptamer.')
parser.add_argument(
    '-t',
    '--threshold',
    type=float,
...
parser.add_argument('-y', '--hybrid', type=str, default="",
                    help='parameter modifying file for hybrid calculations')
